...
Baseline comparison for Flow Matching
"""
...
class DDPMNoiseSchedule:
    ...
    def __init__(self, num_timesteps=1000, s=0.008):
        ...
        self.alphas_cumprod_prev = torch.cat([
            torch.tensor([1.0]), 
            self.alphas_cumprod[:-1]  # ✅ Use self.alphas_cumprod (200 values -> take first 199)
...
def sample_ddpm(
    ...
    """
    Sample from DDPM using DDIM sampling (deterministic when eta=0)
    ...
    Args:
        ...
        class_labels: specific class labels
    ...
    # Create timestep sequence (support for fewer steps than training)
    ...
    with torch.no_grad():
        for i, t in enumerate(tqdm(timesteps, desc="DDPM Sampling")):
            ...
            alpha_bar_t_prev = noise_schedule.alphas_cumprod_prev[t] if t > 0 else torch.tensor(1.0, device=device)
            ...
            if i < len(timesteps) - 1:
                ...
                dir_xt = torch.sqrt(1 - alpha_bar_t_prev - eta**2 * noise_schedule.posterior_variance[t]) * noise_pred
                
                # Random noise component (eta=0 for deterministic)
                noise = torch.randn_like(x) if eta > 0 else 0
                
                # Update x
                x = torch.sqrt(alpha_bar_t_prev) * pred_x0 + dir_xt + eta * torch.sqrt(noise_schedule.posterior_variance[t]) * noise
            else:
                x = pred_x0
...
if __name__ == "__main__":
    ...
    # Test sampling
    samples = sample_ddpm(model, noise_schedule, 4, 10, device='cpu', num_steps=10)
